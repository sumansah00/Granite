...
class TasksControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_should_list_all_tasks_for_valid_user
    get api_v1_tasks_path, headers: @creator_headers
  ...
  def test_shouldnt_create_task_without_title
    post api_v1_tasks_path, params: { task: { title: "", task_owner_id: @creator.id, assigned_user_id: @assignee.id } },
      headers: @creator_headers
  ...
  def test_creator_can_update_any_task_fields
    ...
    put api_v1_task_path(@task.slug), params: task_params, headers: @creator_headers
    assert_response :success
    @task.reload
  ...
  def test_should_destroy_task
    assert_difference "Task.count", -1 do
      delete api_v1_task_path(@task.slug), headers: @creator_headers
    end
  ...
  def test_assignee_shouldnt_destroy_task
    delete api_v1_task_path(@task.slug), headers: @assignee_headers
  ...
  def test_assignee_shouldnt_update_restricted_task_fields
    ...
    assert_no_changes -> { @task.reload.title } do
      put api_v1_task_path(@task.slug), params: task_params, headers: @assignee_headers
      assert_response :forbidden
    end
  end
